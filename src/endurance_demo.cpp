 ...
 */
{
  ...
  ROS_INFO_STREAM("State "
                  ...
                  << (obj->comparePoses(obj->move_group->getCurrentPose().pose, target_pose) ? " succesfully achieved" : " failed"));
  ROS_INFO(" ");
  ...
  ROS_INFO_STREAM("State "
                  ...
                  << (obj->comparePoses(obj->move_group->getCurrentPose().pose, target_pose) ? " succesfully achieved" : " failed"));
  ROS_INFO(" ");
  logger.setMessage(source + ";" + std::to_string(ros::Time::now().toSec()) + ";" + std::to_string(button_no));
  logger.send_data();
